...
fn init() {
    ...
    SCT0.match0().modify(|w| {
        w.set_matchn_l(((4 * TOP) & 0xFFFF) as u16);
        w.set_matchn_h(((4 * TOP) >> 16) as u16);
    });

    SCT0.matchrel0().modify(|w| {
        w.set_reloadn_l(((4 * TOP) & 0xFFFF) as u16);
        w.set_reloadn_h(((4 * TOP) >> 16) as u16);
    });
    ...
    SCT0.match3().modify(|w| {
        w.set_matchn_l(((2 * TOP) & 0xFFFF) as u16);
        w.set_matchn_h(((2 * TOP) >> 16) as u16);
    });

    SCT0.matchrel3().modify(|w| {
        w.set_reloadn_l(((2 * TOP) & 0xFFFF) as u16);
        w.set_reloadn_h(((2 * TOP) >> 16) as u16);
    });

    SCT0.match4().modify(|w| {
        w.set_matchn_l(((3 * TOP) & 0xFFFF) as u16);
        w.set_matchn_h(((3 * TOP) >> 16) as u16);
    });

    SCT0.matchrel4().modify(|w| {
        w.set_reloadn_l(((3 * TOP) & 0xFFFF) as u16);
        w.set_reloadn_h(((3 * TOP) >> 16) as u16);
    }); // Events
    ...
    SCT0.start().modify(|w| w.set_startmsk_l(0));
    ...
    // SCT0.limit().modify(|w| w.set_limmsk_l(1 << 2));
    SCT0.out(OUTPUT_PIN).out_set().modify(|w| w.set_set(1 << 0));
    SCT0.out(OUTPUT_PIN).out_clr().modify(|w| w.set_clr(1 << 1));
    SCT0.out(OUTPUT_PIN).out_clr().modify(|w| w.set_clr(1 << 2));
    SCT0.out(OUTPUT_PIN).out_set().modify(|w| w.set_set(1 << 3));
